...
- (CGSize)setText:(NSString *)text lines:(NSInteger)lines andLineSpacing:(CGFloat)lineSpacing constrainedToSize:(CGSize)cSize andWordsSpace:(CGFloat)wordsSpace{
    ...
    self.lbTextSize = [self p_calculateSizeWithText:text lines:lines font:self.font andLineSpacing:lines constrainedToSize:cSize andWordsSpace:wordsSpace];
...
//真正计算文本占用的size
- (CGSize)p_calculateSizeWithText:(NSString *)text lines:(NSInteger)lines font:(UIFont*)font andLineSpacing:(CGFloat)lineSpacing constrainedToSize:(CGSize)cSize andWordsSpace:(CGFloat)wordsSpace{
    ...
    CGSize textSize = [text boundingRectWithSize:cSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font, NSKernAttributeName : @(wordsSpace)} context:nil].size;
